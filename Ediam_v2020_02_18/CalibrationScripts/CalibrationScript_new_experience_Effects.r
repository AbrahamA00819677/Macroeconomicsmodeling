...
#print as a file
  ...
  calOut$seq<-i
  write.csv(calOut,file.path(dir.calib,"Bootstrap_Out_Exp",paste0("bootsIter_", i, ".csv")),row.names=FALSE)
}

#Compare graphically solution found vs historical record using the following routine
 #load calibration params
   dir.bootstrap<- file.path(dir.calib, "Bootstrap_Out_Exp")
   filenames <- list.files(dir.bootstrap, pattern="*.csv", full.names=FALSE)
   params<-lapply(filenames, function (x) {read.csv(paste(dir.bootstrap,x))})
...
ggsave(filename = file.path(dir.calib, "Calibration_Out", "GDP.pdf"))
...
calib.plot + geom_line()+scale_linetype_manual(values=c(2,1)) + facet_wrap(~ Variable)
ggsave(filename = file.path(dir.calib, "Calibration_Out", "Fossil.pdf"))
#RENTPES
...
calib.plot + geom_line()+scale_linetype_manual(values=c(2,1)) + facet_wrap(~ Variable)
ggsave(filename = file.path(dir.calib, "Calibration_Out", "Renewable.pdf"))
